...
class ReplicaManager(val config: KafkaConfig,
  ...
  def fetchMessages(timeout: Long,
    ...
    info(s"fetchMessages timeout=$timeout, replicaId=$replicaId, fetchMinBytes=$fetchMinBytes")
    ...
    if(timeout <= 0 || fetchInfo.size <= 0 || bytesReadable >= fetchMinBytes || errorReadingData) {
      info("timeout=%d, fetchInfo.size=%d, bytesReadable=%d, fetchMinBytes=%d, errorReadingData=%b".format(timeout, fetchInfo.size, bytesReadable, fetchMinBytes, errorReadingData))
      ...
      info("delayedFetchPurgatory.tryCompleteElseWatch")
